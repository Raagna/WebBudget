...
router.get('/hidden', asyncHandler(async (req, res) => {
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      const inserted = await queryOne(
        ...
        [req.userId, name.trim(), type, isNonEmptyString(icon, 30) ? icon : 'tag']
      );
      res.status(201).json({ id: inserted.id, name: name.trim(), type });
    } catch (err) {
...
router.delete('/:id', asyncHandler(async (req, res) => {
  const category = await queryOne('SELECT id, user_id FROM categories WHERE id = ?', [req.params.id]);
  if (!category || (category.user_id !== null && category.user_id !== req.userId)) {
    return res.status(404).json({ error: 'Category not found' });
  }

  if (category.user_id === req.userId) {
    await run('DELETE FROM categories WHERE id = ? AND user_id = ?', [req.params.id, req.userId]);
  } else {
    await run(
      'INSERT INTO hidden_categories (user_id, category_id) VALUES (?, ?) ON CONFLICT DO NOTHING',
...
router.post('/:id/restore', asyncHandler(async (req, res) => {
  ...
  if (result.rowCount === 0) return res.status(404).json({ error: 'Category was not hidden' });
  res.status(204).end();
}));

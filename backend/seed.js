// Seeds one demo user with ~9 months of clearly fictional transaction
// history, plus sample subscriptions and bills, so the dashboard and
// charts have something meaningful to render immediately.
...
const run = db.transaction(() => {
  ...
  const insertTxn = db.prepare(
    `INSERT INTO transactions (user_id, category_id, amount_cents, type, description, occurred_on, is_recurring, recurring_interval)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  );
  ...
  for (let m = MONTHS - 1; m >= 0; m--) {
    ...
    insertTxn.run(userId, catId('Salary/Income'), 3500 * 100, 'income', 'Monthly salary', isoDate(monthsAgo(m, 1)), 1, 'monthly');

    // Occasional freelance income, roughly every other month
    if (m % 2 === 0) {
      insertTxn.run(userId, catId('Salary/Income'), 42000, 'income', 'Freelance project', isoDate(monthsAgo(m, 15)), 0, null);
    }
  ...
  const subs = [
    ...
    { name: 'News subscription', amount: 8.0, cat: 'Subscription' },
    { name: 'Domain & hosting', amount: 14.0, cat: 'Subscription' },
  ];
  ...
  // Bills (some paid, some upcoming/unpaid)
  const bills = [
    { name: 'Electric bill', amount: 95, cat: 'Utilities', daysOut: 5, paid: false },
    { name: 'Internet bill', amount: 60, cat: 'Utilities', daysOut: 12, paid: false },
    ...
    { name: 'Phone bill', amount: 55, cat: 'Utilities', daysOut: -3, paid: true },
  ];
